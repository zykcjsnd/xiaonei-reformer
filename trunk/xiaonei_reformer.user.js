...
if(XNR.acore==PRESTO) {
	...
	if(XNR.agent==OPERA_EXT) {
		...
		XNR.oexSendRequest=function(msg,handler) {
			do {
				var reqId=Math.floor(Math.random()*100000);
...
if(XNR.agent==MAXTHON) {
	XNR.mxRt=window.external.mxGetRuntime();
}
...
function getTokens() {
	...
	$("script").each(function() {
		...
		if (/get_check\s*:\s*["']?(-?\w+)['"]?/.test(code)) {
			XNR.tokens.reqToken = RegExp.$1;
			n++;
		}
		if (/get_check_x\s*:\s*["']?(\w+)['"]?/.test(code)) {
			XNR.tokens.rtk = RegExp.$1;
			n++;
		}
		if (n == 2) {
			return false;
	...
	if (XNR.tokens.reqToken == null || XNR.tokens.rtk == null) {
		var r = "token" + Math.floor(Math.random()*10000);
		var code = 'var a=document.createElement("div");'+
					'a.id="'+ r +'";'+
					'a.textContent=JSON.stringify({t1:XN.get_check,t2:XN.get_check_x});'+
					'a.style.display="none";'+
					'document.body.appendChild(a);'+
					'var evt=document.createEvent("HTMLEvents");'+
					'evt.initEvent("'+r+'",true,true);'+
					'document.documentElement.dispatchEvent(evt)';
		$(document).bind(r, function() {
			var c = $("#"+r).text();
			var t = JSON.parse(c);
			XNR.tokens.reqToken = t.t1;
			XNR.tokens.rtk = t.t2;
			$(document).unbind(r, arguments.callee);
			$("#"+r).remove();
		});
		$script(code);
	}
...
function removeAds() {
	...
	$wait(1,function() {
		...
		// 右侧广告
		$("#right_side_div").filter("a[href*='apps.renren.com']").remove();
		// 个人主页上的边栏广告
		var t=$(".col-right>.extra-side>div").eq(0);
		t.find("a").each(function(){
			if (/收起/.test(this.textContent)) {
				t.remove();
				return false;
			}
		});
...
function removeBlogMusicPlayer() {
	const target="embed[src*='player.swf'], embed[src*='Player.swf'], object[data*='player']";
	$ban(target);
};
...
// 删除小站上音乐播放器
function removeZhanMusicPlayer() {
	const target=".audio-panel,embed[src*='player.swf']";
	$ban(target);
};

// 隐藏底部工具栏中的好友列表
function removeFriendList() {
	const target="#friends-panel";
	$patchCSS(target+"{display:none !important}");
};

// 隐藏底部工具栏中的人人电台
function removeRadioPlayer() {
	const target="#webpager-miniradio";
	$patchCSS(target+"{display:none !important}");
};
	...
	function addLink(id, action, type, link, regexpr) {
		...
		a.text("全部" + action).addTo(header).bind("click", function() {
			...
			$("#requests_" + id + "_list button[click^='" + id + "_accept:']").each(function() {
				...
				if (param) {
					...
					for (var i = 0; i < param.length; i++) {
						...
						//ilink = ilink.replace(new RegExp("\\$\\{" + i + "\\}", "g"), param[i])
					}
					links.push(ilink);
				}
			});
			...
			if (total > 0) {
				var executer = function() {
					if (links.length > 0) {
						box.text("处理中，请稍候..."+Math.floor((total-links.length)*100/total)+"%");
						var link = links.shift();
						$get(link,executer,null,"POST");
					} else {
...
// 隐藏特定类型/标题新鲜事
function hideFeeds(evt,feeds,mark,badTitles,badSources,badLinks,badIds,goodIds,hideOld,hideDays) {
	...
	if (badLinks) {
		var badLinksArray = badLinks.toLowerCase().split("\n").filter(function(l){return !!l});
	}
	(evt?$(evt.target):$("div.feed-list>article")).filter(function(elem) {
		...
		var fsource=feed.find(".details .legend span.from").text().replace(/\s/g,"").replace(/^通过|发布$/g,"").toLowerCase();
		var flinks=feed.find("a[href]");
		...
		if(badSources && badSourcesArray.length > 0 && fsource) {
			if (badSourcesArray.some(function(s){ return (s.charAt(0)=="!")?(fsource==s.substring(1)):(fsource.indexOf(s)>=0) })) {
				return true;
			}
		}
		if(badLinks && badLinksArray.length > 0 && flinks.exist()) {
			if (badLinksArray.some(function(l){ for(var i=flinks.size()-1;i>=0;i--){if(flinks.get(i).href.toLowerCase().indexOf(l)>=0){return true}}return false })) {
				return true;
			}
		}
		if(hideOld) {
...
// 自动检查提醒新鲜事更新
function autoCheckFeeds(feedFilter,badTitles,badSources,badLinks,badIds,goodIds,checkReply) {
	// 每隔90秒检查一次
	const interval=90;
	...
	(function(fTime) {
		...
		// 定时检查
		window.setTimeout(arguments.callee,Math.floor(interval*1000));
	})(true);
};

// 定时刷新新鲜事列表
function autoReloadFeeds(interval) {
	const code='setInterval(window.newsfeed.reload,'+Math.floor(interval*1000)+')';
	$script(code);
};
	...
	const code="var s=XN.ui.fixPositionElement.prototype;"+
		...
		"s.show=function(){"+
			...
			"if(m && m.className==='menu-dropdown' && t){"+
				...
				"m.style.top=Math.floor(r.bottom)+'px';"+	// opera的定位会出错，必须有这句
				"m.style.left=Math.floor(this.alignType=='3-2'?r.right-m.offsetWidth:r.left)+'px'"+
			"}"+
		"}";
	$script(code);
};
	...
	$wait(1,function() {
		...
		const files={
			...
			"home-frame2-all-min.css":[
				...
				".mentionFrdList .search-Result li.m-autosug-hover{background-color:"+FCOLOR+"}",
...
function addExtraEmotions(emjEmo,nEmo,bEmo,eEmo,fEmo,sfEmo,aEmo,odEmo) {
	...
	if(emjEmo) {
		...
		/* 
		var range = [0xE001, 0xE05A, 0xE101, 0xE15A, 0xE201, 0xE253, 0xE301, 0xE34D, 0xE401, 0xE44C, 0xE501, 0xE537];
		for (var i = 0; i < range.length; i+=2) {
			for (var e = range[i], ed = range[i+1]; e <= ed; e++) {
				var ch = String.fromCharCode(e);
				allEmo[ch]={ kind:999, alt:'', src:"/imgpro/emoji/"+e.toString(16).toLowerCase()+".png", wide:false };
			}
		}
		*/
	}
		...
		"window.$extend=function(obj,attrs){"+
			...
			"if (window.XN && XN.ui && XN.ui.emotions && obj===XN.ui.emotions.prototype){"+
				...
				"obj.formatData=function(data){"+
					"var kind=this.getConfig('emoKind');"+
					"if(kind){"+
						"kind['999']={name:'絵文字',forvip:0};"+
						"kind['1']={name:'阿狸',forvip:0};"+
						"kind['2']={name:'囧囧熊',forvip:0}"+
					"};"+
...
function addBlogHTMLEditor() {
	...
	if ($("#editor_toolbar1 .mce_changeMode").empty()) {
		// 普通模式需要设置下按钮样式
		const css="#editor_editcode{background-position:0 -196px; padding:2px; background-image:url(http://a.xnimg.cn/imgpro/editor/editor.gif); height:24px}#editor_editcode:hover{background-position:0 0}.xiaoneiSkin .mceSeparator{width:0}";
		$patchCSS(css);
	} else {
		// 高级模式，要放到第二行
...
function showImagesInOnePage() {
	...
	} else if(XNR.url.indexOf("/channel-albumshow")!=-1) {
		// 新版公共主页相册
		var baseURL=XNR.url.replace(/[#?].*$/, "")+"?curpage=%%";
		var album=$("#photos .photo-list");
		var items=$(".photo-list li.photo-item");
		var pagerInfo=$("div.p-page-out");
	...
	for(var i=0;i<=maxPage;i++) {
		...
		$get(baseURL.replace("%%",i),function(res,url,page) {
			...
			try {
				if($("#single-column table.photoList").exist()) {
					var photoList=/(<table\s[^>]*?class="([^"]+\s)*photoList(\s[^"]+)*"[^>]*?>[\s\S]+?<\/table>)/.exec(res)[1];
				} else if(XNR.url.indexOf("/channel-albumshow")!=-1) {
					var photoList=/<div\s[^>]*?class="([^"]+\s)*photo-list(\s[^"]+)*"[^>]*?>\s*(<ul[\s\S]+?<\/ul>)/.exec(res)[3];
				} else if(XNR.url.indexOf("/photo/ap/")!=-1) {
					var photoList=/<div\s[^>]*?class="([^"]+\s)*photo-list(\s[^"]+)*"[^>]*?>([\s\S]+?)<\/div>/.exec(res)[3];
				} else if($(".all-photos>.photo-list").exist()) {
					var photoList=/<div\s[^>]*?class="([^"]+\s)*photo-list(\s[^"]+)*"[^>]*?>\s*(<ul>[\s\S]+?<\/ul>)/.exec(res)[3];
				} else if($(".share-photo-main>.photo-list").exist()) {
					var photoList=/<div\s[^>]*?class="([^"]+\s)*photo-list(\s[^"]+)*"[^>]*?>\s*(<ul>[\S\s]+?<\/ul>)/.exec(res)[3];
				} else if($(".group-home>div>.album-list").exist()) {
					var photoList=/(<ul\s[^>]*?class="([^"]+\s)*album-list(\s[^"]+)*">[\s\S]+?<\/ul>)/.exec(res)[1];
				} else {
					var photoList=/<div\s[^>]*?class="([^"]+\s)*photo-list(\s[^"]+)*"[^>]*?>([\s\S]+?)<\/div>/.exec(res)[3];
					var storyList=/<!--start storyList mode-->\s*<div\s[^>]*?class="([^"]+\s)*story-pic(\s[^"]+)*"[^>]*?>([\s\S]+?)<\/div>\s*<!--story mode end-->/.exec(res)[3];
				}
				var pos;
...
function addDownloadAlbumLink(linkOnly,repMode) {
	...
	if($(".function-nav.bottom-operate ul.nav-btn").exist()) {
		...
		downLink2=downLink.clone().css("line-height","23px").css("float","left");
		$(".opera-bar").add(downLink2);
	} else if($(".function-nav.photolist-pager").exist()) {
		// 外链相册
		$("@div").add(downLink).move("before",$(".function-nav.photolist-pager"));
		downLink2=downLink.clone().css("padding-left", "0");
		$(".album-meta .detail").add($("@p").add(downLink2));
	} else if($(".share-operations").exist()) {
		// 从分享相册新鲜事中的相册封面图片进入
		$(".share-operations").add($("@span").attr("class","pipe").text("|")).add(downLink);
		downLink2=downLink.clone().css("padding-left", "0");
		$(".Information").add($("@p").add(downLink2));
		...
		downLink2.move("after", $("#content > .pager-top"));
	} else if($("div.photo-list ul.private-album, div.photo-list ul.public-album").exist()) {
		// 新版公共主页相册。
		// 私有：http://page.renren.com/601545346/channel-albumshow-838844601
		// 公共：http://page.renren.com/600002650/channel-albumshow-874319951
		$(".album-opera").add(downLink, 0);
		downLink2=downLink.clone();
		downLink2.move("after", $(".breadcrumb"));
	} else if($(".group-home ul.album-list").exist()) {
		// 小组相册
		downLink.move("before",$(".group-home>.pagerbox").eq(-1));
		downLink2=downLink.clone();
		downLink2.move("after", $(".group-home>.pagerbox").eq(0));
	} else if($(".pager-bottom,.pagerbottom").exist()) {
		$(".pager-bottom,.pagerbottom").add(downLink.css("lineHeight","22px"),0);
	} else {
		return;
	}
	downLink.bind("click", onclick);
	if (downLink2) {
		downLink2.bind("click", onclick);
	} else {
		downLink2 = $("@dummy");
	}

	$(document.body).attr("downloadable", "yes");
	...
	function onclick(evt) {
		...
		function ajaxInfo(url) {
			$get(url, function(html) {
				...
				try {
					...
					finish();
					$error("addDownloadAlbumLink::ajaxInfo", "相册内容解析失败");
					return;
				}
		...
		if ($(".album-main .review-mode").exist()) {
			// 普通相册，有缩略图模式和评论模式
			// 目前是异步加载新的页数，所以当成是所有图片已经在一页中显示了
		...
		// 普通模式
		links=$(".photo-list span.img a, table.photoList td.photoPan>a, .photo-list>ul>li>a.cover, ul.album-list>li>a.photo-box, .photo-list ul>li>a.picture, li.photo-item>a");
		...
		downLink2.text("分析中...(0/"+totalImage+")");
		var hrefs=[];
		links.each(function(idx) {
			hrefs.push({t:this,l:this.href,i:idx+1});
		});
		...
		$get(a.l,function(html,url,target) {
			try {
				...
				if(html.indexOf("<body id=\"errorPage\">") > 0) {
					return;
				}
				...
				if(src) {
					...
					if(src.photo && src.photo.large) {
						target.img=src.photo.large;
						target.title=src.photo.title;
						return;
					}
				}
				// 新版公共主页相册里看到的
				var src=/shareData\s*:\s*({[^{]+?})/i.exec(html);
				if(src) {
					try {
						src=JSON.parse(src[1]);
						if (src && src.largeurl) {
							target.img=src.largeurl;
				...
				var photo = {
					i: target.i,
					src: target.img,
					title: (target.title || $(target.t).find("img").attr("alt") || "")
				};
				...
				if(!(target.hasOwnProperty("img")) || !/xlarge_|original_|\/p_/.test(target.img)) {
					if (/\/(\d+)\/.*?(\d+)/.test(target.l)) {
						var deepSearch = true;
						var ownerId = RegExp.$1;
						...
						$get("http://photo.renren.com/photo/"+ownerId+"/photo-"+photoId+"/large?xtype=album", function(html,url,photo) {
							...
							if (totalImage == cur) {
								...
								if(downLink.text().match("分析中")) {
									...
									downLink2.text("分析中...("+cur+"/"+totalImage+")");
								}
							}
						}, photo);
					}
		...
		function finish() {
			try {
				...
				downLink2.text("下载当前页图片");
				if($alloc("download_album").length==0) {
					alert("无法获取任何图片的地址");
				} else {
					var failedImages=$(".photo-list span.img a[down],table.photoList td.photoPan>a[down],.photo-list>ul>li>a.cover[down],ul.album-list>li>a.photo-box[down],.photo-list ul>li>a.picture[down], li.photo-item>a[down]");
					...
					if(!title) {
						// 新版公共主页相册
						title=$("#photos>.breadcrumb>span[title]").attr("title");
					}
					if(!title) {
						// 线上活动相册
					...
					if(repMode || XNR.agent==USERSCRIPT || XNR.agent==OPERA_UJS) {
						...
						chrome.extension.sendMessage({action:"album",data:album});
						...
						XNR.mxRt.create("mx.browser").tabs.newTab({
							url: XNR.mxRt.getPrivateUrl() + "album.html",
							position: "afterCurrrent"
						}, function(tab) {
							// 等一秒页面应该加载完毕了
							// 试了tab加载完毕后发消息过来，再从这边发消息过去，没有成功
							setTimeout(function() {
								XNR.mxRt.post("album_for_tab_" + tab.id, album);
							}, 1000);
						});
					} else if(XNR.agent==SOGOU) {
						sogouExplorer.extension.sendMessage({action:"album",data:album});
					}
				}
...
// 显示语音文件下载地址
function showVocalFileLink(evt) {
	if (evt) {
...
function showFullSizeImage(evt,autoShrink,indirect) {
	try {
		...
		if (largeData && /(['"])?large(['"])?:['"]([^'"]+)['"]/.exec(largeData)) {
			...
			if (/\/p_large_|\/large_|\/original|\/xlarge/.test(image)) {
				...
				// 很像是通过普通上传模式上传的图片，直接换成large/original试一下
				_showViewer(evt.pageX,null,imgId,autoShrink,true);
				var testImg = new Image();
				testImg.onerror = function() {
					if (testImg.src.indexOf("/large_") > 0) {
						testImg.src = testImg.src.replace("/large_", "/original_");
					} else {
						_showViewer(evt.pageX,testImg.src,imgId,autoShrink,true);
					}
				};
				testImg.onload = function() {
					_imageCache(imgId,testImg.src);
					_showViewer(evt.pageX,testImg.src,imgId,autoShrink,true);
				};
				testImg.src = image.replace("/main_", "/large_");
				return;
			}
		}
		...
		if((thumbnail.indexOf("tiny_")!=-1 || (thumbnail.indexOf("tiny")!=-1 && thumbnail.indexOf("_")==-1)) && pageURL.indexOf("album")==-1 && pageURL.indexOf("page.renren.com")==-1) {
			...
			if(/profile\.do\?id=600\d{6}/.test(pageURL)) {
				...
				// 一般头像相册
				pageURL="http://photo.renren.com/photo/"+/id=(\d+)/.exec(pageURL)[1]+"/album-profile";
			}
		} else if(pageURL.indexOf("/profile.do?")!=-1) {
			// 直接链接到对方页面的头像图片
			pageURL="http://photo.renren.com/photo/"+/id=(\d+)/.exec(pageURL)[1]+"/album-profile";
		} else if(/\/(\d+)\/profile/.test(pageURL)) {
			pageURL="http://photo.renren.com/photo/"+RegExp.$1+"/album-profile";
		}
		...
		// 公共主页相册图片
		if($page("pages",pageURL) && $page("photo",pageURL)) {
			if (/page\.renren\.com\/\d+\//.test(pageURL)) {
				_loadImage("pages",false,autoShrink,evt,imgId,pageURL,imageDate);
				return;
			}
		}
		...
		if(!indirect) {
			...
			switch(type) {
				case "image":
					...
					break;
				case "pages":
					_getPageImage(pageURL,imgId,autoShrink);
					break;
				case "album":
	...
	function _showMagnifier(target) {
		...
		if (!/\s(avatar|clipImageBig)\s/.test(" "+target.parentNode.className+" ") && rect.height != rect.width) {
			// 高度与宽度不一致的图像
			var rect1=$(target.parentNode).rect(true);
			if(rect1.height<rect.height && rect1.width <= rect.width) {
				rect=rect1;
			}
		}
		...
		if (target.parentNode.tagName=="A" && tpp.find(".myphoto-info").size()==1) {
			...
			node.css({left:Math.floor(rect.right-22)+"px",top:Math.floor(rect.top)+"px"});
		} else {
			node.css({left:Math.floor(rect.right-22)+"px",top:Math.floor(rect.bottom-22)+"px"});
		}
		node.addTo(document);
	...
	function _showViewer(mouseX,src,imgId,shrink,force) {
		...
		if(!$allocated("image_viewer")) {
			if (!shrink) {
				...
				$alloc("image_viewer").image=$("@img").attr("onload","this.parentNode.style.overflowY=(Math.floor(this.height)>Math.floor(window.innerHeight)-10?'scroll':'auto');this.title=''").attr("onerror","this.src='"+errorImage+"';this.title=''").addTo($alloc("image_viewer").viewer);
			} else {
		...
		window.setTimeout(function() {
			...
			if(!src) {
				...
				if(imgId==image.attr("lid")) {
					(shrink?image:viewer).css({maxHeight:(Math.floor(window.innerHeight)-10)+"px",maxWidth:(Math.floor(window.innerWidth)-20)+"px"});	// 边距2，边框宽度3
	...
	// 获取公共主页相册中图片并显示出来
	function _getPageImage(pageURL,imgId,autoShrink) {
		// http://page.renren.com/601545346/channel-photoshow-6780541690
		var src=/\/(\d+)\/.*?(\d+)/.exec(pageURL);
		if (src) {
			$get("http://page.renren.com/jomo/act/album/ajaxgetphoto?pid="+src[1]+"&photoId="+src[2], function(html) {
				try {
					var json = JSON.parse(html);
					_imageCache(imgId,json.data.url);
					_showViewer(null,json.data.url,imgId,autoShrink);
				} catch(ex) {
					_showViewer(null,"error",imgId,autoShrink);
				}
			});
...
function enableShortcutMenu(evt) {
	try {
		...
		var id = t.getAttribute("namecard");
		if (!id || !/^\d+$/.test(id)) {
			if (!/\/profile\.do\?/.test(t.href) && !/\/\/www\.renren\.com\/g\//.test(t.href) && !/\/www\.renren\.com\/\d+$|\/www\.renren\.com\/\d+[?#\/]/.test(t.href) && !/\/\d+\/profile\?/.test(t.href)) {
				return;
			}
		...
		var pages={
			...
			"Ta的头像相册":"http://photo.renren.com/photo/@@/album-profile",
			// 没有上面这个好用
		...
		menu.m=$("@div").html(html).css({position:"absolute",left:Math.floor(rect.left+window.scrollX)+"px",top:Math.floor(rect.bottom+window.scrollY)+"px",backgroundColor:"#EBF3F7",opacity:0.88,padding:"5px 8px",border:"1px solid #5C75AA",zIndex:999987}).addTo(document.body);
	} catch(ex) {
			...
			$get(link+"&curpage=0",function(data, url, i) {
				try {
					...
					if(progress>lastpage) {
						...
						button.value=Math.floor(progress*100/(lastpage+1))+"%";
						i++;
...
function showPhotoAuthorComment() {
	...
	$get(url+"0",function(html) {
		try {
			...
			// 每页30条
			$get(url+Math.floor(o.commentCount/30),function(html) {
				try {
					if(!html) {
						return;
					}
...
function showMusicFileLink() {
	var a = $("@a").text("下载").attr("id","dlLink");
	var objPath;
	if ($page("musicbox")) {
		a.css("line-height","26px").addTo($(".cur_list_action"));
		hijack("window.musicBox");
	} else if ($page("fm")) {
		a.css("line-height","42px").addTo($(".operation"), 0);
		hijack("XN.app.WebRadio");
	} else {
		if (!$("#home-radio-bar").exist()) {
			if (!$allocated("@showMusicFileLink")) {
				$alloc("@showMusicFileLink", {n:0});
			}
			var counter = $alloc("@showMusicFileLink");
			if (counter.n < 20) {
				counter.n++;
				setTimeout(arguments.callee, 1000);
			}
		...
		$script("XN.radio.home.seedLoad()");
		var extraCode = "var $=function(a,b){return (b?b:document).querySelector(a)};"+
			"var t=$('#webpager-miniradio');"+
			"if (t) {"+
				"if (!$('#dlLink',t)) {"+
					"var a=document.createElement('a');"+
					"a.id='dlLink';"+
					"a.style.lineHeight='26px';"+
					"a.textContent='当前歌曲文件下载地址';"+
					"var div=document.createElement('div');"+
					"div.style.textAlign='center';"+
					"div.appendChild(a);"+
					"$('.d-person',t).appendChild(div);"+
					// 删除广告位腾出空间
					"var ad=$('.ad-clause',t);"+
					"if (ad) {"+
				...
				"}"+
			"}";
		hijack("XN.radio", extraCode);
	}

	function hijack(objPath, extraCode) {
		var code="var count=0;"+
		"(function(){"+
			"var m="+objPath+";"+
			"if(!m || !m.player){"+
			...
			"if(m.player.oplay){"+
				"return;"+
			"}"+
			"m.player.oplay=m.player.play;"+
			"m.player.play=function(url){"+
				(extraCode || "") +
				"var t=document.getElementById('dlLink');"+
				"if(t) {"+
					"t.href=url;"+
				"}"+
				"return "+objPath+".player.oplay.apply(this,arguments);"+
			"};"+
		"})()";
		$script(code);
	}
		...
		$("@a").text("清空列出的分享").attr("href", "#nogo").addTo($("@div").css({padding:"5px","text-align":"center",cursor:"pointer"}).move("before",$(".j-share-list"))).bind("click",function() {
			...
			if (total > 0) {
				var deleter = function() {
					if (ids.length > 0) {
						box.text("处理中，请稍候..."+Math.floor((total-ids.length)*100/total)+"%");
						var id = ids.shift();
...
function notifyFriendship() {
	...
	$storage("friends_" + XNR.userId, function(dataString) {
		...
		$get('http://friend.renren.com/recommendSelector.do?p={"init":true,"uid":true,"uhead":true,"uname":true,"group":false,"net":true,"param":{}}&t='+Math.random(), function(html) {
			...
			function localXHR() {
				token = "DONE" + Math.floor(Math.random() * 1000000);
			...
			function compare() {
				...
				$get("http://www.renren.com/newnamecard?uid=" + oldFriends[fi].id, function(html, url, idx) {
					...
					if (idx < oldFriends.length) {
						...
						dialog.addTo(document.body).css({"top":Math.floor(window.innerHeight-dialog.prop("offsetHeight"))/2+"px","left":Math.floor(window.innerWidth-dialog.prop("offsetWidth"))/2+"px"});
						$storage("friends_" + XNR.userId, JSON.stringify({friends:curFriends, time:now}));
					}
				}, fi);
			}
...
function updatedNotify(notify,lastVersion) {
	...
	if(lastVer<368) {
		...
		if(rect) {
			$("@div").attr("id","xnr_optip").html('<div style="border-color:transparent transparent red transparent;border-style:solid;width:0;height:0;top:'+Math.floor(rect.bottom-14)+'px;border-width:8px;left:'+Math.floor((rect.left+rect.right)/2-8)+'px;position:absolute;z-index:100000"></div><div style="background:red;-moz-border-radius:3px;border-radius:3px;top:'+Math.floor(rect.bottom+2)+'px;padding:5px 10px;left:'+Math.floor(rect.right-118)+'px;color:white;font-weight:bold;position:absolute;min-width:104px;-moz-box-shadow:2px 2px 5px #292929;-webkit-box-shadow:2px 2px 5px #292929;box-shadow:2px 2px 5px #292929;z-index:99999;text-align:center;-moz-user-select:none;-khtml-user-select:none;cursor:default">点击这里进行设置</div>').addTo($("body")).bind("click",function() {
				$("#xnr_optip").remove();
			});
		}
	}
...
function main(savedOptions) {
	...
	var optionMenu={
		"清理页面":[
			{
				...
				page:"blog,lover_blog,page_blog"
			},{
				text:"##去除公共主页上音乐播放器",
				ctrl:[{
				...
				text:"##去除小站上音乐播放器",
				ctrl:[{
					id:"removeZhanMusicPlayer",
					value:false,
					fn:[{
						name:removeZhanMusicPlayer,
						stage:0,
						fire:true,
					}],
				}],
				...
				text:"##隐藏底部人人电台",
				ctrl:[{
					id:"removeRadioPlayer",
					value:false,
					fn:[{
						name:removeRadioPlayer,
						stage:0,
						fire:true,
					}],
				}]
		...
		"处理新鲜事":[
			{
				...
				ctrl:[{
					...
					fn:[{
						name:hideFeeds,
						stage:1,
						args:[null,"@feedGroup","@markFeedAsRead","@forbiddenFeedTitle","@forbiddenFeedSource","@forbiddenFeedLink","@forbiddenFeedId","@trustedFeedId","@hideOldFeeds","@oldFeedDays"],
				...
				ctrl:[
					{
						type:"info",
						value:"当新鲜事中任意一个链接包含任意一行中的内容时即被隐藏，忽略大小写。"
					},{
						type:"br"
					},{
						id:"forbiddenFeedLink",
						value:"",
						type:"edit",
						style:"margin-left:5px;width:310px;height:50px"
					}
				],
				page:"feed,profile"
			},{
				...
				ctrl:[
					{
						...
						fn:[{
							...
							stage:3,
							fire:true,
							args:["@feedGroup","@forbiddenFeedTitle","@forbiddenFeedSource","@forbiddenFeedLink","@forbiddenFeedId","@trustedFeedId","@checkFeedReply"]
						}]
					},{
						id:"checkFeedReply",
						type:"subcheck",
		...
		"辅助功能":[
			{
				text:"##启用隐藏表情项##包括绘文字表情##包括自然风光表情##包括网络流行语表情##包括节日事件表情##包括人物表情##包括抢沙发表情##包括商业广告表情##包括过期事件表情",
				ctrl:[
					{
						id:"addExtraEmotions",
						value:true,
						fn:[{
							...
							stage:2,
							args:["@emojiEmo","@natureEmo","@bwEmo","@eventEmo","@figureEmo","@sfEmo","@advEmo","@odEmo"],
							fire:true
						}],
					},{
						type:"subcheck",
	...
	if (window.self == window.top || window.top.location.host.indexOf("renren.com") < 0) {
		...
		entry.find("a").bind("click",function() {
			menu.show().css({"top":Math.floor(window.innerHeight-menu.prop("offsetHeight"))/2+"px","left":Math.floor(window.innerWidth-menu.prop("offsetWidth"))/2+"px"});
	...
	$wait(1,function() {
		var eventId="XNR"+Math.floor(Math.random()*10000);
...
function $page(category,url) {
	const pages={
		...
		photo:"/photo\\.renren\\.com/getphoto\\.do|/photo\\.renren\\.com/gettagphoto\\.do|/photo\\.renren\\.com/photo/sp/|/photo\\.renren\\.com/photo/[0-9]+/photo-|/photo\\.renren\\.com/photo/[0-9]+/[^/]+/photo-|/page\\.renren\\.com/[^/]+/photo/|page\\.renren\\.com/[^/]+/channel-photoshow-|event\\.renren\\.com/event/[0-9]+/[0-9]+/photo/[0-9]+|lover\\.renren\\.com/photo/",	// 照片
		album:"photo\\.renren\\.com/getalbum|photo\\.renren\\.com/.*/album-[0-9]+|photo\\.renren\\.com/photo/.*/album-profile||page\\.renren\\.com/.*/album|page\\.renren\\.com/.*/channel-albumshow-|/photo/album\\?|photo\\.renren\\.com/photo/ap/|event\\.renren\\.com/event/[0-9]+/photos|event\\.renren\\.com/event/[0-9]+/stars|lover\\.renren\\.com/album/",	// 相册
		friend:"/friend\\.renren\\.com/|#!//friend/",	// 好友
		...
		musicbox:"/music\\.renren\\.com/box",	// 人人爱听播放器
		fm:"/fm\\.renren\.com/",	// 人人电台
...
function $popup(title,content,geometry,stayTime,popSpeed) {
	...
	var maxHeight=Math.floor(node.prop("clientHeight"));
	node.css("height","0px");
	// 展开
	window.setTimeout(function () {
		try {
...
function $save(name,value) {
	...
	switch(XNR.agent) {
		...
		case OPERA_EXT:
			...
			break;
		case MAXTHON:
			XNR.mxRt.storage.setConfig("options", opts);
			break;
	}
};
	...
	if (typeof data === "function") {
		...
		switch(XNR.agent) {
			case USERSCRIPT:
				...
				break;;
			case CHROME:
				chrome.extension.sendMessage({action:"storage", pref:name}, function(response) {
					data(response.data);
				});
				break;
			case SOGOU:
			...
			case OPERA_EXT:
				...
				break;
			case MAXTHON:
				data(XNR.mxRt.storage.getConfig(name));
				break;
		}
	} else {
		switch(XNR.agent) {
			...
			case OPERA_EXT:
				...
				break;
			case MAXTHON:
				XNR.mxRt.storage.setConfig(name, data);
				break;
		}
	}
}
...
function $get(url,func,userData,method) {
	...
	if(method=="POST" && url.indexOf("renren.com") > 0) {
		// XN.get_check & XN.get_check_x
		var reqToken = XNR.tokens.reqToken || $("input[type='hidden'][name='requestToken']").val();
		var rtk = XNR.tokens.rtk || $("input[type='hidden'][name='_rtk']").val();
	...
	switch(XNR.agent) {
		...
		case CHROME:
			if(func==null) {
				chrome.extension.sendMessage({action:"get",url:url,method:method});
			} else {
				chrome.extension.sendMessage({action:"get",url:url,method:method},function(response) {
		...
		case SAFARI:
			...
			if (/renren\.com$/.test(domain)) {
				var requestId="xnr_ajax_"+Math.floor(Math.random()*1000000);
...
function $feedType(feed) {
	...
	if(stats && (/_\d+_(\d+)_\d+$/.test(stats) || /_\d+_(\d+)_$/.test(stats) || /sfet=(\d+)/.test(stats))) {
		...
		var mtype=Math.floor(ntype/100);
		switch(mtype) {
			case 0:
				// 评论日志/照片:0
				return "comment";
...
function $pager(pager) {
	...
	if(pager.exist() && pager.find("li").exist()) {
		try {
			curpage=parseInt(pager.find("ol.pagerpro li.current a, ol.p-page-list li.current a").text())-1;
			// 在线活动相册的总数是放在注释中的
			...
			// 新版公共主页
			var p=pager.find("ol.p-page-list>li:last-child>a");
			if (p.exist() && /(\d+)$/.test(p.attr("href"))) {
...
switch(XNR.agent) {
	...
	case OPERA_EXT:
		...
		break;
	case MAXTHON:
		var opts=XNR.mxRt.storage.getConfig("options");
		try {
			opts=JSON.parse(opts);
		} catch(ex) {
			opts={};
