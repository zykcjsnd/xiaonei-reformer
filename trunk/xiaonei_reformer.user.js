...
function main(savedOptions) {
	...
	// 添加类别页，绑定提示信息事件
	menu.find(".pages").add($(categoryPages)).bind("mouseover,mouseout",function(evt) {
		var t=$(evt.target);
		if(!t.attr("tooltip")) {
			return;
		}
		...
		if($allocated(tag)) {
			$alloc("optionsMenu_tooltip").remove();
