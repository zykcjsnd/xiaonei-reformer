...
// 清除广告
function removeAds() {
	var ads=".ad-bar, .banner, .wide-banner, .adimgr, .blank-bar, .renrenAdPanel, .side-item.template, .rrdesk, .login-page .with-video .video, .login-page .side-column .video, .ad-box-border, .ad-box, .ad, .share-ads, .kfc-side, .imAdv, #sd_ad, #showAD, #huge-ad, #rrtvcSearchTip, #top-ads, #bottom-ads, #main-ads, #n-cAD, #webpager-ad-panel, #ad, .box-body #flashcontent, div[id^='ad100'], .share-success-more>p>a>img[width='280']";
	$ban(ads);
...
function main(savedOptions) {
	...
	menu.find(".pages").add($(categoryPages)).bind("mouseover",function(evt) {
		...
		evt.stopPropagation();
		var rect=t.rect();
		var tip=$alloc(tag,$("@div").css({maxWidth:"300px",background:"#FFFFBF",border:"1px solid #CFCF3D",position:"fixed",zIndex:"200001",padding:"6px 8px 6px 8px",fontSize:"13px",left:(rect.right+3)+"px","top":(rect.bottom+3)+"px"}));
...
PageKit.prototype={
	...
	remove:function(safe) {
		this.each(function() {
			if((!safe || this.childElementCount==0) && this.parentNode) {
